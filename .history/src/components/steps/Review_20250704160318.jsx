...
export default function Step7Review() {
  ...
  // Load submission state from localStorage on mount
  ...
  const [submittedDate, setSubmittedDate] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('applicationSubmittedDate') || null;
    }
    return null;
  });
  ...
  useEffect(() => {
    if (submittedDate) {
      localStorage.setItem('applicationSubmittedDate', submittedDate);
    }
  }, [submittedDate]);
